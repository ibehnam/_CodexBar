#!/usr/bin/env bash
# Build CodexBar with ad-hoc signing (no Apple Developer account required)
# This app will run on your local machine but cannot be distributed
...
if [[ -d ".build/$CONF/Sparkle.framework" ]]; then
  ...
  # Ad-hoc sign Sparkle and all nested components (no Developer ID required)
...
# Ad-hoc sign widget extension if present
...
# Finally ad-hoc sign the app bundle itself
codesign --force --sign "-" \
  --entitlements "$APP_ENTITLEMENTS" \
  "$APP"

echo "Created $APP (ad-hoc signed - runs locally but not distributable)"
